"""
prozorro_to_neo4j_v8.py  —  Завантаження тендерів Prozorro у Neo4j
══════════════════════════════════════════════════════════════════════
ЗМІНА v8: endpoint /api/search/tenders приймає POST, не GET.
Надсилаємо тіло запиту з фільтром buyer[0]=24604168.
...
def build_body(page: int) -> dict:
    ...
    Пробуємо кілька варіантів структури — один із них підійде.
    """
    return {
        "buyer":  [TARGET_EDRPOU],          # масив ЄДРПОУ
        "page":   page,
        "limit":  PAGE_SIZE,
    }
...
def main():
    print("╔══════════════════════════════════════════════════════╗")
    print("║   prozorro_to_neo4j_v8  —  Завантаження тендерів   ║")
    print("╚══════════════════════════════════════════════════════╝\n")
    print(f"  ЄДРПОУ  : {TARGET_EDRPOU}  ({TARGET_NAME})")
    print(f"  Endpoint: POST {SEARCH_URL}\n")

    # ── Перевірка: показуємо сиру відповідь API ──────────────────
    print("[1] Перевірка endpoint (POST)...")
    body = build_body(1)
    print(f"    Тіло запиту: {json.dumps(body, ensure_ascii=False)}")

    try:
        r = requests.post(SEARCH_URL, json=body, headers=HEADERS, timeout=20)
        print(f"    HTTP статус : {r.status_code}")
        print(f"    Content-Type: {r.headers.get('Content-Type','?')}")
        print(f"\n    Відповідь (перші 800 символів):")
        print("    " + r.text[:800].replace("\n", "\n    "))
        print()

        if r.status_code != 200:
            print("    ✗ Помилка. Дивись відповідь вище.")
            return
    ...
    with driver.session() as session:
        for page in range(1, MAX_PAGES + 1):
            ...
            if not tenders:
                if page == 1:
                    print("  ⚠ Перша сторінка порожня!")
                    print("  Сира відповідь:")
                    print("  " + json.dumps(raw, ensure_ascii=False, indent=2)[:600])
                else:
...
""")
        ...
        print("\n  ⚠ Нуль. Скопіюй весь вивід і надішли.\n")
